...
def leftLegStarting(t, xi, yi):
	...
	if(t < t0/4):
		x2 = xi
		y2 = (yi-l0/2) + 3*(-l0/2)*((t)**2)/((t0/4)**2) - 2*(-l0/2)*((t)**3)/((t0/4)**3)
	elif(t < 3*t0/4):
		x2 = xi
		y2 = yi-l0
	else:
		x2 = xi + 3*(x0/4)*((t-3*t0/4)**2)/((t0/4)**2) - 2*(x0/4)*((t-3*t0/4)**3)/((t0/4)**3)
		y2 = (yi - l0/2) - (l0/2)*np.sqrt(abs(1-((x2-xi)/(x0/4))**2))
	z2 = z0
	...
	if(t < t0/8):
		...
		x4 = xi + 3*(x0/2)*((t-t0/8)**2)/((6*t0/8)**2) - 2*(x0/2)*((t-t0/8)**3)/((6*t0/8)**3)
	else:
		x4 = xi + x0/2
	y4 = yi+l0/2
	z4 = (l3)*np.sqrt(abs(1-((x4-(xi + x0/4))/(x0/4))**2))
...
def rightLegWalking(t, xi, yi):
	...
	if(t < t0/4):
		x2 = xi + 3*(x0/4)*((t)**2)/((t0/4)**2) - 2*(x0/4)*((t)**3)/((t0/4)**3)
	elif(t < 3*t0/4):
		x2 = xi + x0/4
	else:
		x2 = (xi + x0/4) + 3*(x0/4)*((t-3*t0/4)**2)/((t0/4)**2) - 2*(x0/4)*((t-3*t0/4)**3)/((t0/4)**3)
	
	y2 = (yi-l0/2) + (l0/2)*np.sqrt(abs(1-((x2-(xi+x0/4))/(x0/4))**2))
	z2 = z0
	...
	if(t < t0/8):
		...
		x1 = xi-x0/4 + 3*(x0)*((t-t0/8)**2)/((6*t0/8)**2) - 2*(x0)*((t-t0/8)**3)/((6*t0/8)**3)
	else:
		x1 = xi + 3*x0/4
	y1 = yi - l0/2
	z1 = (l3)*np.sqrt(abs(1-((x1-(xi+x0/4))/(x0/2))**2))
...
def leftLegWalking(t, xi, yi):
	...
	if(t < t0/8):
		...
		x4 = xi-x0/4 + 3*(x0)*((t-t0/8)**2)/((6*t0/8)**2) - 2*(x0)*((t-t0/8)**3)/((6*t0/8)**3)
	else:
		x4 = xi + 3*x0/4
	y4 = yi+l0/2
	z4 = (l3)*np.sqrt(abs(1-((x4-(xi + x0/4))/(x0/2))**2))
...
# rightLegMotorPosition = [0, 0.0, 1.1221245872819505, -1.8613982115132142, 0.7392736242312636, -0.0]
# leftLegMotorPosition = [0, 0.0, 0.739273624231264, -1.8613982115132142, 1.12212458728195, -0.0]

# for i in range(len(rightLegMotor)):
# 	p.setJointMotorControl2( bodyIndex=robot, jointIndex=rightLegMotor[i], controlMode=p.POSITION_CONTROL, targetPosition=rightLegMotorPosition[i])


# for i in range(len(leftLegMotor)):
# 	p.setJointMotorControl2( bodyIndex=robot, jointIndex=leftLegMotor[i], controlMode=p.POSITION_CONTROL, targetPosition=leftLegMotorPosition[i])
...
# rightLegMotorPosition = [0, .2, -0.5, 1, -0.5, -.2]
# leftLegMotorPosition = [0, .2, -0.5, 1, -0.5, -.2]
...
# rightLegMotorPosition = [.3, .2, -0.5, 1, -0.5, -.2]
# leftLegMotorPosition = [0.3, .2, -0.5, 1, -0.5, -.2]
